...
const plugin = {
  ...
  init: async (bot, { buttonUrl, mess, api, apikey }) => {
    bot.onText(/^\/douyin(?: (.+))?$/, async (msg, match) => {
      ...
      if (!inputText) {
        bot.sendMessage(chatId, 'Input Link! Example /douyin https://v.douyin.com/ikq8axJ/ ', { reply_to_message_id: msg.message_id });
        return;
      }
      bot.sendMessage(chatId, mess.wait, { reply_to_message_id: msg.message_id });
      try {
        const apis = await fetch(api + '/api/download/douyin?apikey=' + apikey + '&url=' + encodeURIComponent(inputText));
